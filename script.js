  ...
  jsbimaru_.prototype = {
    ...
    tilesClick: function() {
      if (jsb.currentMode == 1) {
        ...
        if (jsb.currentCell.is(".set")) {
          ...
          $("#canvas tbody tr td:nth-child(" + (col + 1) + ")>div").each(
            function(i, el) {
              if (el.className == "") {
                el.className = "water";
    ...
    canvasClick: function() {
      if (jsb.currentMode == 1) {
        if (
          ...
          $(this)
            .children("div")
            .attr("class", jsb.tileSet[jsb.currentTile]);
          var c = jsb.getCoords($(this));
          jsb.currentGrid[c[1]][c[0]] =
            jsb.currentTile == 13 ? 0 : jsb.currentTile;
          jsb.incMoves();
        } else if (
          $(this)
            .children("div")
    ...
    keyPress: function(e) {
      ...
      if (jsb.currentMode == 1) {
        ...
        var td = $("#canvas td.selected>div")
            .attr("class", jsb.tileSet[n])
            .parent(),
          c = jsb.getCoords(td);
    ...
    keyDown: function(e) {
      switch (e.keyCode) {
        case 37: // left arrow
          ...
          jsb.currentCell = jsb.currentCell
            .parent()
            .removeClass("selected")
            .prev()
            .addClass("selected")
    ...
    countShips: function() {
      ...
      for (i = 0; i < cg.length; i++) {
        for (j = 0; j < cg[i].length; j++) {
          ...
          if (cg[i][j] == 52) {
            for (h = 1; h < 8; h++) {
              if (cg[i][j + h] == 53) continue;
              else if (cg[i][j + h] == 54)
                $("div.ship" + (h + 1) + ":not(.done):first", s).addClass(
                  "done"
                );
              else break;
            }
    ...
    checkSolveState: function() {
      ...
      row
        ...
        .each(function(i) {
          ...
          if (
            row
              .find("td:nth-child(" + (i + 1) + ")>div[class^=ship]")
              .size() !==
            parseInt(
              row
                .last()
...
$(function() {
  ...
  $.get("bim/list.txt", function(str) {
    ...
    if (gEasy == "") $("#modal-start .easy").hide();
    else $("#modal-start ul.games.easy").html(gEasy);

    if (gNorm == "") $("#modal-start .normal").hide();
    else $("#modal-start ul.games.normal").html(gNorm);

    if (gHard == "") $("#modal-start .hard").hide();
    else $("#modal-start ul.games.hard").html(gHard);
